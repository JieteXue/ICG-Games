# [file name]: games/take_coins/ui.py
"""
Take Coins Game UI Components - Dark Display for Invalid Positions
"""
...
class TakeCoinsUI:
    ...
    def __init__(self, screen, font_manager):
        ...
        self.visible_positions = 8  # 可见的位置数量
    ...
    def draw_coin_stacks(self, game_logic, position_buttons, scroll_buttons):
        ...
        for button in position_buttons:
            if (button.position_index >= self.scroll_offset and 
                ...
                # Calculate visual position based on scroll
    ...
    def _draw_single_coin_stack(self, button, visible_index, game_logic):
        ...
        if button.selected:
            ...
            # 无效的不可移动位置 - 暗色
            base_color = (40, 50, 60)       # 暗色底座
            border_color = (60, 70, 80)     # 暗色边框
        ...
        if button.selected:
            ...
            pos_text_color = TEXT_COLOR if button.enabled else (100, 110, 120)  # 暗色文字
    ...
    def _draw_coin_graphics(self, x, base_y, coin_count, is_valid, is_selected):
        ...
        # 确定硬币颜色基于状态
        if not is_valid:
            # 无效位置（边界或相邻位置硬币不足）- 暗色硬币
            coin_color = (70, 70, 80)       # 暗灰色硬币
            border_color = (50, 50, 60)     # 更暗的边框
            highlight_color = (90, 90, 100, 100)  # 暗色高光
        elif is_selected:
            # 选中的位置 - 高亮
            coin_color = (255, 225, 50)     # 亮金色
            border_color = (218, 165, 32)
            highlight_color = (255, 255, 255, 150)
        else:
            # 正常位置
            coin_color = (255, 200, 50)     # 金色
        ...
        if coin_count > max_display_coins:
            count_color = TEXT_COLOR if is_valid else (100, 100, 110)  # 暗色计数
    ...
    def create_buttons(self):
        ...
        buttons = {
            ...
            "restart": Button(SCREEN_WIDTH//2 - 80, 620, 160, 45, "New Game", 
    ...
    def create_position_buttons(self, coins, valid_positions, selected_position):
        ...
        for i, coin_count in enumerate(coins):
            button = PositionButton(0, 0, 80, 120, f"Position {i}", self.font_manager)
            button.position_index = i
            # 只有有效位置可以选中和移动
            button.enabled = (i in valid_positions)  # 只有有效位置可以选中
...
class PositionButton:
    """Position selection button - only valid positions can be selected"""
    ...
    def __init__(self, x, y, width, height, text, font_manager):
        ...
        self.hovered = False
        self.selected = False
        self.enabled = False  # 默认禁用，只有有效位置才启用
        self.position_index = 0
        self.coin_count = 0
    
    def update_hover(self, mouse_pos):
        """Update hover state based on current scroll position"""
        pass
    
    def is_clicked(self, event):
        """Check if clicked - position is handled by the game logic"""
        return False
